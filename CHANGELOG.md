...
- CI tests now pass on Linux — replaced shell glob `src/**/*.test.ts` (broken on dash/sh) with `find src -name '*.test.ts'`.

## [0.3.2] - 2026-03-22

### Fixed

- `skillpod` CLI now starts correctly — `enablePositionalOptions()` added to the root program so the `install` command's `passThroughOptions()` no longer throws on startup.

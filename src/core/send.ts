...
import { pullRegistry, pushRegistry } from './registry-git.js';
import { skillsInternals } from './skills.js';
import { pathExists } from '../utils/filesystem.js';
import { logger, type Logger } from '../utils/logger.js';
import type { SkillPodConfig } from '../types/config.js';
import type { RegistryGitDependencies } from './registry-git.js';
...
export async function sendSkill(
  ...
  if (!(await checkExists(skillMdPath))) {
    throw new Error(`No SKILL.md found in ${sourcePath}. A valid skill directory must contain a SKILL.md file.`);
  }
